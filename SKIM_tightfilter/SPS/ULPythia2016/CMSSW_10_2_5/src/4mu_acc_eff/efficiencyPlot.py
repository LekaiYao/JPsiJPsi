...
#     inFiles[inFileName] = TFile(inFileName, "READ")
#     if inFiles[inFileName].Get("rootuple/oniaTree"):
#         inTrees.append(inFiles[inFileName].Get("rootuple/oniaTree"))
# for inTree in inTrees:
#     print(inTree)
inTree = TChain("rootuple/oniaTree")
for inFilePath in inFileList:
    inTree.AddFile(inFilePath)
...
entryCut = "GEevt_valid && GEevt_passAcc && GEevt_fourMuMass > 7.5 && "# + "3.1415926 - fabs(fabs(GEJpsi1_phi - GEJpsi2_phi) - 3.1415926) > 1.57 && "# deltaPhi cut
...
nFidEvt += inTree.GetEntries("GEevt_valid && GEevt_fourMuMass > 7.5 && " + fidJpsi1Cut + fidJpsi2Cut + "1")
...
with open(outPlotDir + outFile, 'w') as f:
    n_Jpsi = 0
    n, m = len(ptBin) - 1, len(yBin) - 1
    for i in range(m):
        for j in range(n):
            n_Jpsi += mBin_Jpsi[i][j]
    f.write("{} {} {} {} {}\n".format(n, m, nAccEvt, n_Jpsi, nFidEvt))
    l = ""
    for pt in ptBin:
        l += str(pt) + ' '
    l += '\n'
