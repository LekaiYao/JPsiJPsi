...
if [ "${DPS_MC_CLOSURE_INSIDE_EL7:-0}" != "1" ]; then
  exec /cvmfs/cms.cern.ch/common/cmssw-el7 --command-to-run \
    "export DPS_MC_CLOSURE_INSIDE_EL7=1; exec $0 ${1:-tests/atlas_data_driven_sps/results/fdps_systematics_final_v1/dps_mc_full_workflow_closure_v1} ${2:-absy1p2} ${3:-0.164184277213}"
fi

OUTPUT_DIR="${1:-tests/atlas_data_driven_sps/results/fdps_systematics_final_v1/dps_mc_full_workflow_closure_v1}"
SINGLE_J_MODE="${2:-absy1p2}"
REPO="/eos/home-l/leyao/26JJ/JPsiJPsi"
CMSSW_SRC="${REPO}/Data/ULntuple16/CMSSW_10_6_20/src"
MODEL="${CMSSW_SRC}/NtupleAnalyzer/Model_4D_tot.root"
REFERENCE="${REPO}/tests/atlas_data_driven_sps/results/route9p1_nominal_dps_mc/WeightDPS_current.root"
...
root -l -b -q \
  "tests/atlas_data_driven_sps/build_jpsi12_candidates.cpp+(\"${OUTPUT_DIR}/jpsi12_candidates.root\",\"dps_mc\")" \
  > "${OUTPUT_DIR}/build_candidates.log" 2>&1
for slot in 1 2; do
  if [ "${SINGLE_J_MODE}" = "global_calibrated" ]; then
    root -l -b -q \
      "tests/atlas_data_driven_sps/fit_single_jpsi_global_calibrated_splot.cpp+(${slot},\"${OUTPUT_DIR}/jpsi12_candidates.root\",\"${MODEL}\",\"${OUTPUT_DIR}/splot/jpsi${slot}_sweights.root\")" \
    ...
    root -l -b -q \
      "tests/atlas_data_driven_sps/fit_single_jpsi_category_splot.cpp+(${slot},\"absy\",1.2,\"${OUTPUT_DIR}/jpsi12_candidates.root\",\"${MODEL}\",\"${OUTPUT_DIR}/splot/jpsi${slot}_sweights.root\")" \
      > "${OUTPUT_DIR}/fit_jpsi${slot}.log" 2>&1
...
root -l -b -q \
  "tests/atlas_data_driven_sps/build_crossslot_splot_mixed.cpp+(0,0,\"${OUTPUT_DIR}/splot/jpsi1_sweights.root\",\"${OUTPUT_DIR}/splot/jpsi2_sweights.root\",\"${OUTPUT_DIR}/mixed_dps_allpairs.root\")" \
  > "${OUTPUT_DIR}/build_mixing.log" 2>&1
root -l -b -q \
  "tests/atlas_data_driven_sps/fit_pp_2d_adaptive.cpp+(\"${OUTPUT_DIR}/pp_data_2d\",\"${REFERENCE}\",true,false,true)" \
  > "${OUTPUT_DIR}/fit_pp.log" 2>&1
grep -q '^accepted_fits=12$' "${OUTPUT_DIR}/pp_data_2d/summary.txt"
root -l -b -q \
  "tests/atlas_data_driven_sps/build_2d_templates_adaptive.cpp+(\"${OUTPUT_DIR}/templates_2d\",\"${OUTPUT_DIR}/pp_data_2d/fit_results.csv\",\"${OUTPUT_DIR}/mixed_dps_allpairs.root\",1.8,1.5707963267948966,0.0,true)" \
