...
if [ "${FULL_BOOTSTRAP_INSIDE_EL7:-0}" != "1" ]; then
  exec /cvmfs/cms.cern.ch/common/cmssw-el7 --command-to-run \
    "export FULL_BOOTSTRAP_INSIDE_EL7=1; exec $0 $1 $2 $3 ${4:-legacy14} ${5:-absy1p2}"
fi
...
case "${BINNING_MODE}" in
  legacy14)
    PP_FIT_CALL="tests/atlas_data_driven_sps/fit_pp_2d_adaptive.cpp+(\"${OUTPUT_DIR}/pp_data_2d\",\"${OUTPUT_DIR}/input/WeightData_mJJ7p5.root\")"
    EXPECTED_FITS=14
    PP_REFIT_LABEL="adaptive_14_cells_legacy"
    ;;
  nominal12_mergeddy)
    PP_FIT_CALL="tests/atlas_data_driven_sps/fit_pp_2d_adaptive.cpp+(\"${OUTPUT_DIR}/pp_data_2d\",\"${OUTPUT_DIR}/input/WeightData_mJJ7p5.root\",true,false,true)"
    EXPECTED_FITS=12
...
MODEL="${REPO}/Data/ULntuple16/CMSSW_10_6_20/src/NtupleAnalyzer/Model_4D_tot.root"
SOURCE_CANDIDATES="${REPO}/tests/atlas_data_driven_sps/results/route9p3_crossslot_splot/jpsi12_candidates.root"
SOURCE_DATA="${REPO}/tests/atlas_data_driven_sps/results/unified_input/WeightData_mJJ7p5.root"
...
run_stage resample root -l -b -q \
  "tests/atlas_data_driven_sps/build_full_bootstrap_replica_input.cpp+(${REPLICA_ID},${BASE_SEED},\"${INPUT_DIR}\",\"${SOURCE_CANDIDATES}\",\"${SOURCE_DATA}\")"
for slot in 1 2; do
  if [ "${SINGLE_J_MODE}" = "global_calibrated" ]; then
    run_stage "global_calibrated_splot_jpsi${slot}" root -l -b -q \
      "tests/atlas_data_driven_sps/fit_single_jpsi_global_calibrated_splot.cpp+(${slot},\"${INPUT_DIR}/jpsi12_candidates.root\",\"${MODEL}\",\"${SPLOT_DIR}/jpsi${slot}_sweights.root\")" \
    ...
    run_stage "category_splot_jpsi${slot}" root -l -b -q \
      "tests/atlas_data_driven_sps/fit_single_jpsi_category_splot.cpp+(${slot},\"absy\",1.2,\"${INPUT_DIR}/jpsi12_candidates.root\",\"${MODEL}\",\"${SPLOT_DIR}/jpsi${slot}_sweights.root\")" \
      > "${OUTPUT_DIR}/fit_jpsi${slot}.log" 2>&1
...
run_stage all_cross_event_mixing root -l -b -q \
  "tests/atlas_data_driven_sps/build_crossslot_splot_mixed.cpp+(0,0,\"${SPLOT_DIR}/jpsi1_sweights.root\",\"${SPLOT_DIR}/jpsi2_sweights.root\",\"${MIXED}\")" \
...
run_stage nominal_cr_normalization root -l -b -q \
  "tests/atlas_data_driven_sps/build_2d_templates_adaptive.cpp+(\"${TEMPLATE_DIR}\",\"${PP_DIR}/fit_results.csv\",\"${MIXED}\",1.8,1.5707963267948966,0.0,true)" \
