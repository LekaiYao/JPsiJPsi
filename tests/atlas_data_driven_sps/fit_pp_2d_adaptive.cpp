...
FitCell fitCell(TTree &tree, const FitCell &definition) {
    ...
    TFile modelFile(kModelPath, "READ");
    ...
    for (int attempt = 0; attempt < 6; ++attempt) {
        ...
        std::unique_ptr<RooFitResult> result(pdf->fitTo(
            *fitData, Extended(true), Save(true), SumW2Error(false), Offset(true),
...
void fit_pp_2d_adaptive(
    const char *outputDir =
        "tests/atlas_data_driven_sps/results/pp_data_2d_adaptive14",
    const char *dataPath =
    ...
    for (const FitCell &definition : definitions) {
            ...
            FitCell cell = fitCell(*tree, definition);
            cells.push_back(cell);
