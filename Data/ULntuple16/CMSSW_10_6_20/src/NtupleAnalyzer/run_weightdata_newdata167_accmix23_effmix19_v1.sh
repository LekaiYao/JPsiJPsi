...
run_tag="${1:-newdata167_accmix23_effmix19_lumi36p684_root640_v1_20260902}"
...
if [[ "${WEIGHTDATA_NEWDATA_INSIDE_EL7:-0}" != "1" ]]; then
    exec /cvmfs/cms.cern.ch/common/cmssw-el7 --command-to-run \
        "export WEIGHTDATA_NEWDATA_INSIDE_EL7=1; exec ${script_path} ${run_tag}"
fi
...
weight_name="WeightData_newdata167_accmix23_effmix19_v1_20260902.root"
metadata_name="WeightData_newdata167_accmix23_effmix19_v1_20260902.metadata.txt"
...
scratch="$(mktemp -d "${scratch_parent}/jpsijpsi_weightdata_newdata167.XXXXXX")"
cd "${cmssw_src}"
...
start_epoch="$(date +%s)"
root -l -b -q rephrase.cpp > "${log_dir}/rephrase.log" 2>&1
end_epoch="$(date +%s)"

test -s WeightData.root
root -l -b -q -e 'TFile f("WeightData.root","READ"); TTree *t=dynamic_cast<TTree *>(f.Get("data")); if(f.IsZombie() || !t || !t->GetBranch("evt_weight")) gSystem->Exit(2); const Long64_t n=t->GetEntries(); const Long64_t nonfinite=t->GetEntries("!TMath::Finite(evt_weight)"); const Long64_t nonpositive=t->GetEntries("evt_weight<=0"); const double minimum=t->GetMinimum("evt_weight"); const double maximum=t->GetMaximum("evt_weight"); printf("entries=%lld\nnonfinite_weights=%lld\nnonpositive_weights=%lld\nminimum_weight=%.17g\nmaximum_weight=%.17g\n",n,nonfinite,nonpositive,minimum,maximum); if(n<=0 || nonfinite || nonpositive || maximum>1000.) gSystem->Exit(3);' > "${log_dir}/weightdata_qa.txt" 2>&1
...

{
    ...
    echo "task=weightdata_newdata167_accmix23_effmix19_v1"
...
echo "WEIGHTDATA_NEWDATA_COMPLETE output=${input_dir}/${weight_name} sha256=${output_sha}"
