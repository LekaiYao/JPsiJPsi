...
run_tag="${1:-newdata167_accmix23_effmix19_lumi36p684_pairsym_cseed50_root640_strategy2_nominalonly_v1_20260902}"
...
source_weight_tag="newdata167_accmix23_effmix19_lumi36p684_pairsym_cseed50_root640_v1_20260902"
...
if [[ -e "${result_dir}" ]]; then
    echo "Refusing to reuse result tag: ${result_dir}" >&2
    exit 17
fi

mkdir -p "${input_dir}" "${total_dir}/logs"
...
for required in Model_4D_tot.root Fit_4D_tot_native_asymptotic_unseeded.root; do
    test -s "${required}"
done
...

{
    ...
    echo "task=total_4d_newdata167_accmix23_effmix19_pairsym_cseed50_root640_strategy2_nominalonly_v1"
    ...
    echo "fits=nominal only (the Gaussian-core reference model is not an input to this GOF test)"
    ...
    sha256sum Fit_4D_tot.cpp Plot_4D.hpp Model_4D_tot.root \
        Fit_4D_tot_native_asymptotic_unseeded.root total_fit_summary.csv
} > run.metadata.txt

echo "TOTAL4D_PAIRSYM_STRATEGY2_NOMINAL_COMPLETE result=${total_dir}/total_fit_summary.csv"
