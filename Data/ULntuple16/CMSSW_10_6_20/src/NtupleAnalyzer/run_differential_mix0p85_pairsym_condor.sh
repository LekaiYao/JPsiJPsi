...
weight_data="${result_dir}/inputs/${weight_name}"
...
cd "${analysis_dir}"
worker_cache="${_CONDOR_SCRATCH_DIR:-/tmp}/xdg-cache-diff-${job_index}"
...
required_files=(
    "${weight_data}"
    "${total_dir}/Model_4D_tot.root"
    "${total_dir}/Model_4D_tot_ref.root"
    "${total_dir}/Fit_4D_tot_native_asymptotic_unseeded.root"
    "${total_dir}/Fit_4D_tot_ref_native_asymptotic_unseeded.root"
    Fit_4D_diff.cpp Fit_4D_diff_cpp.so
    Fit_valid.cpp Fit_valid_cpp.so Plot_4D.hpp
)
...
root -l -b -q -e "gSystem->Load(\"Fit_4D_diff_cpp.so\"); Fit_4D_diff(\"${variable}\",${minimum},${maximum},false,true,\"${campaign_tag}\",\"${weight_data}\",\"${total_dir}\",false,true);" > "${nominal_log}" 2>&1
root -l -b -q -e "gSystem->Load(\"Fit_4D_diff_cpp.so\"); Fit_4D_diff(\"${variable}\",${minimum},${maximum},true,true,\"${campaign_tag}\",\"${weight_data}\",\"${total_dir}\",false,true);" > "${reference_log}" 2>&1
root -l -b -q -e "gSystem->Load(\"Fit_valid_cpp.so\"); Fit_valid(\"${variable}\",${minimum},${maximum},\"${campaign_tag}\",\"${total_dir}\");" > "${validation_log}" 2>&1
...

{
    ...
    echo "fit_parameters=all total shape parameters fixed; five yields float"
    echo "retry=failed fits may use Strategy(2)+Offset(true); failed lower-boundary n_Sig_Comb and/or n_Comb_Comb may be fixed to zero"
    echo "worker_runtime=precompiled ROOT libraries loaded directly; per-job XDG cache"
