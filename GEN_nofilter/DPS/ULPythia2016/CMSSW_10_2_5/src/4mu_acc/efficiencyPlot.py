...
inFileDir = "/eos/user/c/chensh/JPsiJPsi/GEN_nofilter/DPS/ULPythia2016/CMSSW_10_2_5/src/4mu_acc/direct/"  # chensh Pythia8 GEN samples (local direct/ is empty)
# inFileDir = "direct/"
inFileList = ["DPS_2016_JJ_{}.root".format(i) for i in range(1, 11)]
inFiles = {}
inTrees = []
...
for inTree in inTrees:
    ...
    # nEvt += inTree.GetEntries(evtMassCut + "1" + fidCut_Jpsi1) + inTree.GetEntries(evtMassCut + "1" + fidCut_Jpsi2)
    # nAccEvt += inTree.GetEntries(evtMassCut + "1" + fidCut_Jpsi1 + etaCut_Jpsi1 + ptCut_Jpsi1) + inTree.GetEntries(evtMassCut + "1" + fidCut_Jpsi2 + etaCut_Jpsi2 + ptCut_Jpsi2)
# print(nEvt, nAccEvt)
for ptIdx in range(len(ptBin) - 1):
    for yIdy in range(len(yBin) - 1):
        ...
        for inTree in inTrees:
            ...
            nEta_Jpsi += inTree.GetEntries(binCut_Jpsi1 + etaCut_Jpsi1) + inTree.GetEntries(binCut_Jpsi2 + etaCut_Jpsi2)
            nPt_Jpsi += inTree.GetEntries(binCut_Jpsi1 + etaCut_Jpsi1 + ptCut_Jpsi1) + inTree.GetEntries(binCut_Jpsi2 + etaCut_Jpsi2 + ptCut_Jpsi2)
