...
# 样式与 official 对齐: CMS Preliminary + J/Psi(gen.) 轴 + colorbar "Number"。
...
ptB = array('d', [i for i in range(10, 24)] + [24, 26, 28, 30, 35, 40])
yB  = array('d', [-2, -1.75, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 1.75, 2])
n, m = len(ptB) - 1, len(yB) - 1

# GEN-only DPS samples (chensh shared dir; acceptance-stage samples, not the SKIM ones)
...
ch.Draw("%s:%s>>+hBin" % (rJ1_y, rJ1_pt), em, "goff")
ch.Draw("%s:%s>>+hBin" % (rJ2_y, rJ2_pt), em, "goff")
ch.Draw("%s:%s>>+hAcc" % (rJ1_y, rJ1_pt), em + eta1 + pt1, "goff")
ch.Draw("%s:%s>>+hAcc" % (rJ2_y, rJ2_pt), em + eta2 + pt2, "goff")
maxN = hBin.GetMaximum()
print("filled: hBin max=%d ; hAcc entries=%d" % (maxN, hAcc.GetEntries()))
...
def draw(h, outname):
    ...
    h.GetZaxis().SetRangeUser(0., maxN)
