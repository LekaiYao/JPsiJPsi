...
void loadAcc(bool statis) {
    ...
    ifstream accFile("Data_driven/inputs/acceptance_sps_full10_v1.txt");
...
void calculate_mass_extrapolation(
    const char *output="Data_driven/postprocess/mass_extrapolation.txt") {
    ...
    ofstream summary(output);
    summary << "status=complete\n"
            << "definition=epsilon_m=sigma_DPS_mJJ_gt_7p5_over_sigma_DPS_no_mass_cut\n"
            << "mixing=GEN_pair_id_slot1_x_slot2_different_source_events\n"
            << "pool1=" << pools[0].size() << "\n"
            << "pool2=" << pools[1].size() << "\n"
            << "all_cross_event_pairs=" << nTried << "\n"
            << "pass_mJJ_gt_7p5=" << nEvent << "\n"
            << setprecision(15)
            << "epsilon_m=" << static_cast<double>(nEvent) / nTried << "\n"
            << "acceptance_weighted_closure=" << closure << "\n"
            << "acceptance_weighted_closure_jackknife_stat=" << closureStat << "\n";
    summary.close();
    return;
}
