...
EVENT_MIX_SUMMARY="${CENTRAL}/templates_2d/summary.txt"
...
CR_DIR="${OUTPUT}/cr_variations_half_mc"
...
EVENT_MIX_F_DPS=$(awk -F= '$1=="f_dps"{print $2}' "${EVENT_MIX_SUMMARY}")
test -n "${EVENT_MIX_F_DPS}"
...
root -l -b -q \
  "Data_driven/build_2d_templates_adaptive.cpp+(\"${HALF_TEMPLATE}\",\"${NOMINAL_FIT}\",\"${HALF_INPUT}\",1.8,${PHI90},0.0,false,\"dps_reconstruction_mc_fixed_seed_half\")" \
  > "${OUTPUT}/logs/half_nominal.log" 2>&1
test "$(awk -F= '$1=="mixed_pairs"{print $2}' "${HALF_TEMPLATE}/summary.txt")" \
  -eq 2584
test "$(awk -F= '$1=="dps_template_source"{print $2}' \
  "${HALF_TEMPLATE}/summary.txt")" = "dps_reconstruction_mc_fixed_seed_half"
...
EVENT_MIX_SIGNED_SHIFT=$(awk -v e="${EVENT_MIX_F_DPS}" -v h="${HALF_F_DPS}" \
  'BEGIN{printf "%+.12g",e-h}')
EVENT_MIX_RELATIVE_PERCENT=$(awk -v e="${EVENT_MIX_F_DPS}" -v h="${HALF_F_DPS}" \
  'BEGIN{printf "%+.12g",100*(e-h)/h}')
...
printf 'nominal,1.8,%.12g,12,%.12g,0,%s,12\n' "${PHI90}" "${HALF_F_DPS}" \
  "$(awk -F= '$1=="negative_sps_bins"{print $2}' "${HALF_TEMPLATE}/summary.txt")" \
  >> "${VARIATIONS}"
...
run_cr() {
  ...
  root -l -b -q \
    "Data_driven/build_2d_templates_adaptive.cpp+(\"${result_dir}\",\"${pp_dir}/fit_results.csv\",\"${HALF_INPUT}\",${dy},${phi},0.0,false,\"dps_reconstruction_mc_fixed_seed_half\")" \
    > "${OUTPUT}/logs/${label}.log" 2>&1
  test "$(awk -F= '$1=="cells"{print $2}' "${result_dir}/summary.txt")" \
    -eq "${cells}"
  test "$(awk -F= '$1=="dps_template_source"{print $2}' \
    "${result_dir}/summary.txt")" = "dps_reconstruction_mc_fixed_seed_half"
  value=$(awk -F= '$1=="f_dps"{print $2}' "${result_dir}/summary.txt")
  shift=$(awk -v v="${value}" -v f="${HALF_F_DPS}" \
...
{
  echo "status=nominal_complete_user_confirmed"
  echo "artifact=nominal_fdps_fixed_half_dpsmc_with_cr_systematic"
  echo "method=fixed_seed_exact_half_DPS_reconstruction_MC_template_CR_normalized"
  echo "nominal_definition=user_confirmed_fixed_seed_exact_half_DPS_MC"
  echo "nominal_user_confirmed_at=2026-09-02"
  echo "f_dps=${HALF_F_DPS}"
  echo "f_sps=${HALF_F_SPS}"
  echo "stat_definition=absolute_shift_nominal_fixed_seed_exact_half_DPS_MC_vs_full_DPS_MC"
  echo "stat_seed=${SEED}"
  ...
  echo "stat_full_reference_f_dps=${FULL_F_DPS}"
  echo "stat_full_reference_f_sps=${FULL_F_SPS}"
  echo "stat_signed_nominal_minus_full=${HALF_SIGNED_SHIFT}"
  echo "stat_uncertainty=${STAT_ABS}"
  echo "stat_scope=user_defined_single_fixed_half_sample_DPS_MC_template_shift_only"
  echo "cr_systematic_definition=zero_leakage_phi70_phi110_dy2p4_envelope_using_nominal_fixed_half_DPS_MC"
  ...
  echo "event_mixing_role=cross_check_not_nominal"
  echo "event_mixing_interpretation=supports_basic_DPS_MC_plausibility_not_shape_identity"
  echo "event_mixing_f_dps=${EVENT_MIX_F_DPS}"
  echo "event_mixing_signed_shift_from_nominal=${EVENT_MIX_SIGNED_SHIFT}"
  echo "event_mixing_relative_shift_percent=${EVENT_MIX_RELATIVE_PERCENT}"
...
HALF_SELECTION_SHA=$(sha256sum "${HALF_SELECTION_CSV}" | awk '{print $1}')
{
  ...
  echo "full_dpsmc_reference_tag=${FULL_DPSMC_TAG}"
  echo "dps_reference_tag=${DPS_REFERENCE_TAG}"
  echo "cr_pp_source_tag=${CR_PP_SOURCE_TAG}"
  echo "event_mixing_crosscheck_tag=${CENTRAL_TAG}"
  ...
  echo "nominal_half_sample_input=${HALF_INPUT}"
  echo "nominal_half_sample_input_sha256=${HALF_INPUT_SHA}"
  echo "nominal_half_sample_selected_entries_sha256=${HALF_SELECTION_SHA}"
  echo "nominal_definition=fixed_seed_exact_half_DPS_reconstruction_MC_template_CR_normalized"
  echo "nominal_user_confirmed_at=2026-09-02"
  echo "stat_seed=${SEED}"
  echo "stat_engine=std_mt19937_64"
  echo "stat_sampling=exact_half_without_replacement"
  echo "cr_variations=phi70_phi110_dy2p4_zero_SPS_leakage"
  echo "cr_template=fixed_seed_exact_half_DPS_reconstruction_MC"
  echo "event_mixing_role=cross_check_not_nominal"
  echo "event_mixing_interpretation=supports_basic_DPS_MC_plausibility_not_shape_identity"
  ...
  sha256sum "${CENTRAL}/run.metadata.txt" "${NOMINAL_FIT}" \
    ...
    Data_driven/run_nominal_dpsmc_half.sh
...
printf 'nominal_complete_user_confirmed\n' > "${OUTPUT}/complete.marker"
echo "DATA_DRIVEN_DPSMC_HALF_NOMINAL_COMPLETE tag=${OUTPUT_TAG} f_DPS=${HALF_F_DPS} stat=${STAT_ABS} cr_up=${CR_UP} cr_down=${CR_DOWN}"
