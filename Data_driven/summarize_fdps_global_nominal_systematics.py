...
def main():
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: summarize_fdps_global_nominal_systematics.py "
            "SYSTEMATICS_DIR BOOTSTRAP_DIR"
        )
    base = pathlib.Path(sys.argv[1])
    bootstrap_base = pathlib.Path(sys.argv[2])
    repo = pathlib.Path(__file__).resolve().parents[2]
    if (base / "summary.txt").exists():
        raise RuntimeError(f"refusing to overwrite {base / 'summary.txt'}")
    nominal = float(read_kv(base / "baseline" / "summary.txt")["f_dps"])
    bootstrap_path = bootstrap_base / "summary.txt"
    ...
    if abs(float(bootstrap["nominal_f_dps"]) - nominal) > 5e-10:
        raise RuntimeError("bootstrap and systematics nominal f_DPS do not match")
    ...
    if (projections.get("status") not in
            {"complete", "complete_pending_user_confirmation"} or
    ...
    summary = [
        "status=global_nominal_stat_and_systematics_complete_pending_user_confirmation",
        ...
        "statistical_uncertainty_status=fixed500_global_bootstrap_pending_user_confirmation",
        ...
        "category_group_status=pending_user_confirmation",
        f"cr_group_up={cr_up:.12g}",
        f"cr_group_down={cr_down:.12g}",
        "cr_group_status=pending_user_confirmation",
        ...
        "dps_mc_full_workflow_closure_status=pending_user_confirmation",
    ...
    sources = [
        repo / "Data_driven/fit_single_jpsi_category_splot.cpp",
        repo / "Data_driven/fit_single_jpsi_global_calibrated_splot.cpp",
        repo / "Data_driven/fit_pp_2d_adaptive.cpp",
        repo / "Data_driven/build_2d_templates_adaptive.cpp",
        repo / "Data_driven/run_dps_mc_full_workflow_closure.sh",
        repo / "Data_driven/run_fdps_global_nominal_systematics.sh",
        repo / "Data_driven/run_global_nominal_projection_crosscheck.sh",
        repo / "Data_driven/build_component_trees.cpp",
        repo / "Data_driven/fit_component_variables.cpp",
        repo / "Data_driven/summarize_component_fits.cpp",
        repo / "Data_driven/fit_1d_component_templates.cpp",
        repo / "Data_driven/summarize_fdps_global_nominal_systematics.py",
        repo / "Data_driven/summarize_bootstrap.py",
    ]
    metadata = [
        "artifact=fdps_global_nominal_method_systematics",
        "runtime=ROOT_6_40_02",
        ...
        "category_systematic_status=pending_user_confirmation",
        "bootstrap=fixed500_global_complete_500_of_500_no_replacement_pending_user_confirmation",
    ...
    (base / "complete.marker").write_text(
        "analysis_complete_pending_user_confirmation\n", encoding="utf-8"
    )
    print("\n".join(summary))
